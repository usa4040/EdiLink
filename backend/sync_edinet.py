...
def sync_documents(
    ...
    print(f"Syncing documents from {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}...")
...
def sync_issuer_names(csv_path: str = None):
    ...
    sec_code_col = [c for c in df.columns if '証券コード' in c][0] if any('証券コード' in c for c in df.columns) else None
    ...
    for _, row in df.iterrows():
        ...
        if code and code != 'nan':
            ...
            sec_code = str(row[sec_code_col]).strip() if sec_code_col and pd.notna(row[sec_code_col]) else None
            ...
            for csv_file in csv_files:
                try:
                    with zf.open(csv_file) as f:
                        ...
                        try:
                            df = pd.read_csv(io.BytesIO(content), encoding='utf-16', sep='\t', on_bad_lines='skip')
                        except:
                            try:
                                df = pd.read_csv(io.BytesIO(content), encoding='cp932', on_bad_lines='skip')
                            except:
                                continue
                        ...
                        if '項目名' in df.columns and '値' in df.columns:
                            # 株券等保有割合を探す（最大の値を取得）
                            ratio_mask = df['項目名'].str.contains('株券等保有割合', na=False) & ~df['項目名'].str.contains('直前|欄外|増減', na=False)
                            ratio_rows = df[ratio_mask]
                            if len(ratio_rows) > 0:
                                for val in ratio_rows['値']:
                                    try:
                                        ratio = float(str(val).replace('%', '').replace(',', '').strip())
                                        # 保有割合は0-1の範囲で格納されている場合は100倍する
                                        if 0 < ratio <= 1:
                                            ratio = ratio * 100  # パーセンテージに変換
                                        if 0 < ratio <= 100:
                                            if result['holding_ratio'] is None or ratio > result['holding_ratio']:
                                                result['holding_ratio'] = ratio
                                    except:
                                        continue
                            
                            # 保有株券等の数（総数）を探す（最大の値を取得）
                            shares_mask = df['項目名'].str.contains('保有株券等の数（総数）', na=False) & ~df['項目名'].str.contains('欄外', na=False)
                            shares_rows = df[shares_mask]
                            if len(shares_rows) > 0:
                                for val in shares_rows['値']:
                                    try:
                                        shares = int(str(val).replace(',', '').replace('株', '').strip())
                                        if shares > 0:
                                            if result['shares_held'] is None or shares > result['shares_held']:
                                                result['shares_held'] = shares
                                    except:
                                        continue
                            
                            # 保有目的を探す
                            purpose_mask = df['項目名'].str.contains('保有の目的', na=False) & ~df['項目名'].str.contains('欄外', na=False)
...
def sync_holding_details(filer_edinet_code: str = None, limit: int = None, year: int = None):
    ...
    with get_db_session() as db:
        ...
        query = db.query(Filing).filter(
            Filing.csv_flag == True
        ).outerjoin(HoldingDetail).filter(
            HoldingDetail.id == None
        )
...
def main():
    ...
    parser.add_argument("--days", type=int, default=365, help="過去何日分を同期するか（デフォルト: 365）")
    parser.add_argument("--filer", type=str, default=None, help="特定の提出者EDINETコードでフィルタ（例: E04948）")
