...
class TestExtractHoldingDataFromCsv:
    ...
    def test_extract_holding_data_success(self) -> None:
        """正常系: 保有データが正しく抽出される"""
    ...
    def test_extract_holding_data_with_percentage_symbol(self) -> None:
        """保有割合に%記号が含まれる場合"""
    ...
    def test_extract_holding_data_ratio_as_decimal(self) -> None:
    ...
    def test_extract_holding_data_empty_zip(self) -> None:
        """空のZIPファイルの場合"""
        zip_buffer = io.BytesIO()
    ...
    def test_extract_holding_data_no_csv_in_zip(self) -> None:
        """ZIP内にCSVファイルがない場合"""
        zip_buffer = io.BytesIO()
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
    ...
    def test_extract_holding_data_invalid_csv_format(self) -> None:
        """CSVの形式が不正な場合"""
        # "項目名"と"値"の列がない
    ...
    def test_extract_holding_data_partial_data(self) -> None:
        """一部のデータのみ存在する場合"""
        csv_content = "項目名\t値\n株券等保有割合（％）\t10.5\n"  # sharesとpurposeなし
        zip_content = create_test_zip(csv_content)
